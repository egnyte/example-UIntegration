...
//    decorators: decorators
//}
Egnyte.appInit = function (options, callback) {
    ...
    Egnyte.plugin("uintegrate", function pluginClosure(root, resources) {
        ...
        function respond(body) {
            messages.sendMessage(window.parent, channel, "complete", body, remoteDomain);
        }

        function error(body) {
            messages.sendMessage(window.parent, channel, "error", body, remoteDomain);
        }
        ...
        init.promise.then(function (data) {
            try {
                if (data.domain) {
                    remoteDomain = "https://" + data.domain;
                    egnyteInstance.setDomain(remoteDomain);
                    callback(egnyteInstance, data, respond, error);
                } else {
                    error("No domain in init message");
                }
            } catch (e) {
                error(e.message);
            }
    ...
    var egnyteInstance = Egnyte.init("Domain name not available", options);
}
